...
// var utilServer = require('./util-server.js');
...
exports.register = function(commander) {
    ...
    // 加载fis插件，启动服务。
    function fisServerStart() {
        var options = {
            type: 'node'
        };
        var app = fis.require('server-node');

        utilServer.serverInfo(options);
        app.start(options, function(error, info) {
            ...
            // options.qrcode && qrcode.generate((options.https ? 'https' : 'http') + '://' + util.hostname + (options.port == 80 ? '/' : ':' + options.port + '/'));
